...
Found the original code in this location...
https://code.google.com/archive/p/rs22812/

I made some changes to bring it up to date. Today versus 2009 there are
even fewer machines have real serial ports. 
Luckily I have an Optiplex-7010 and a few older machines which has them. 
So I am able to use this code to run the meter to collect some measurements
 remotely.
But if you don't have a serial port, there are serial to USB adapters available.

Updated version that works and was tested with...
...
2. setDTR ( used as self.sp.setDTR) depreciated, using self.sp.dtr = False 
style instead.
3. Removed all ord() calls.

Additional Code
4. In this archive created port-test.py which scans serial ports and reports
open ones. The file is worth reading if you are struggling to get ports to
work.

Notes

I had to add myself to the dialout group or else to code errors. Calling the
code with sudo does not help unless you pip install pyserial to work with
root.

sudo usermod -a -G dialout <username>

I also tried zmeter out but, it did not respond when run. Sat there with nothing
showing up. But it might be an alternative to try out and modify.
https://zmeter.sourceforge.net/

Requirements

Requires serial (https://pyserial.readthedocs.io/en/latest/pyserial.html)


pip install pyserial


* Logging version, logs output to file.

rs22812_linux_py3_l.py

- Defaults to output.log
- Time and Date formatting changed, 
  dashes between DD-MM-YYYY for easier parsing.
  Comma between date and time.

Help for Command Line Options
-----------------------------

Usage: rs22812_linux_py3_l.py [options]

rs22812 - A python interface for the Radio Shack 22-812 digital multimeter.

Options:
  -h, --help            show this help message and exit
  -p PORT, --port=PORT  port device string: examples:  Unix-like: /dev/ttyS0,
...
if __name__ == "__main__":
    ...
    parser.add_option("-l", "--log",
        ...
        type = "str",
        default = 'output.log',
        help = "Log to a file. Takes filename as paramter. (default %default)"
    )
    ...
    while True:
        ...
        with open(logfile, 'a') as file:
            # Write the current time and reading to the file
            file.write(TimeNow() + " [%d] %s\n" % (count, r))
